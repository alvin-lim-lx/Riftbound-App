...
class RulesBasedAI {
    ...
    decide(state) {
        const legalActions = (0, GameEngine_1.getLegalActions)(state, this.playerId);
    ...
    scoreAction(action, state) {
        ...
        switch (action.type) {
            ...
            case 'Attack': {
                ...
                const enemyUnits = targetBf?.units.filter(id => state.allCards[id]?.ownerId !== this.playerId) ?? [];
                let enemyMight = 0;
            ...
            case 'PlayUnit': {
                ...
                if (def.keywords.includes('Ambush'))
                    score += 8;
                if (def.keywords.includes('Assault'))
                    score += 8;
                if (def.keywords.includes('Deflect'))
                    score += 6;
                if (def.keywords.includes('Hunt'))
                    score += 12;
            ...
            case 'MoveUnit': {
                const { cardInstanceId, toBattlefieldId } = action.payload;
...
exports.RulesBasedAI = RulesBasedAI;
function createAI(playerId) {
