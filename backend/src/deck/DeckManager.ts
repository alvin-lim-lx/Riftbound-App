...
import type { Deck, DeckValidation, Domain } from '../../shared/src/types';
...
function rowToDeck(row: Record<string, unknown>): Deck {
  return {
    ...
    legendId: row['legend_id'] as string,
    chosenChampionCardId: (row['chosen_champion_card_id'] as string) ?? null,
    cardIds: JSON.parse(row['card_ids'] as string),
    runeIds: JSON.parse(row['rune_ids'] as string),
    battlefieldIds: JSON.parse(row['battlefield_ids'] as string),
    sideboardIds: JSON.parse(row['sideboard_ids'] as string),
    isAiDeck: (row['is_ai_deck'] as number) === 1,
...
export class DeckManager {
  ...
  static create(
    ...
    `).run(
      ...
      JSON.stringify(battlefieldIds),
      JSON.stringify(sideboardIds),
      isAiDeck ? 1 : 0,
      now,
      now,
    );
  ...
  static update(
    ...
    const existing = db.prepare('SELECT * FROM decks WHERE id = ?').get(id) as
      | Record<string, unknown>
      | undefined;
    ...
    `).run(
      merged.name,
      merged.legendId,
      merged.chosenChampionCardId,
      JSON.stringify(merged.cardIds),
      JSON.stringify(merged.runeIds),
      JSON.stringify(merged.battlefieldIds),
      JSON.stringify(merged.sideboardIds),
      now,
      id,
    );
  ...
  static delete(id: string): boolean {
    const info = db.prepare('DELETE FROM decks WHERE id = ?').run(id);
    return info.changes > 0;
  }

  static getRandomAiDeck(): Deck | undefined {
    const rows = db
  ...
  static extractChampionTag(name: string): string {
  ...
  static domainsMatch(
    ...
    if (cardDomains.includes('Colorless')) return true;
    if (isBattlefield && cardDomains.length === 0) return true;
    if (cardDomains.length === 0) return true;
    if (legendDomains.length === 0) return true;
  ...
  static validate(
    deck: Omit<Deck, 'id' | 'playerId' | 'createdAt' | 'updatedAt'>,
  ): DeckValidation {
    const errors: string[] = [];
    const warnings: string[] = [];

    const legend = CARDS[deck.legendId];
    if (!legend) {
      errors.push(`Legend card "${deck.legendId}" not found in card database.`);
    } else if (legend.type !== 'Legend') {
      errors.push(
        `Selected legend "${legend.name}" is type '${legend.type}', expected 'Legend'.`,
      );
    }

    const legendDomains: Domain[] = legend?.domains ?? [];
    const legendTag = legend?.championName ?? null;

    const chosenChampion = deck.chosenChampionCardId ? CARDS[deck.chosenChampionCardId] : null;
    if (!chosenChampion) {
      if (deck.chosenChampionCardId) {
        errors.push(
          `Chosen Champion card "${deck.chosenChampionCardId}" not found in card database.`,
        );
      }
    } else {
      if (chosenChampion.superType !== 'Champion') {
        errors.push(
          `Chosen Champion "${chosenChampion.name}" is superType '${chosenChampion.superType}', expected 'Champion'.`,
        );
      }
      if (legendTag) {
        const champTag =
          chosenChampion.tags?.includes(legendTag) ? legendTag :
          chosenChampion.tags?.[0] ?? DeckManager.extractChampionTag(chosenChampion.name);
        if (champTag !== legendTag) {
          errors.push(
            `Chosen Champion "${chosenChampion.name}" has tag "${champTag}" — does not match legend "${legend.name}" tag "${legendTag}".`,
          );
        }
      }
      if (
        ...
        errors.push(
          `Chosen Champion "${chosenChampion.name}" has domains [${chosenChampion.domains.join(', ')}] — does not share any domain with legend "${legend.name}" [${legendDomains.join(', ')}].`,
        );
      }
    }
    ...
    if (mainDeckSize !== 40) {
      errors.push(
        `Main deck has ${mainDeckSize} cards, must be exactly 40 (Chosen Champion is part of the main deck and extracted during setup).`,
      );
    }
    ...
    for (const cardId of deck.cardIds) {
      ...
      if (!['Unit', 'Spell', 'Gear'].includes(card.type)) {
        errors.push(
          `Main deck card "${card.name}" is type '${card.type}', expected Unit/Spell/Gear.`,
        );
      }

      if (legend && !DeckManager.domainsMatch(legendDomains, card.domains)) {
        errors.push(
          `Card "${card.name}" has domains [${card.domains.join(', ')}] — does not share any domain with legend "${legend.name}" [${legendDomains.join(', ')}].`,
        );
      }
    ...
    if (signatureCount > 3) {
      errors.push(
        `Deck has ${signatureCount} Signature cards, maximum is 3 total (103.2.d).`,
      );
    }
    ...
    const seenRuneNames: Record<string, number> = {};
    for (const runeId of deck.runeIds) {
      const rune = CARDS[runeId];
      if (!rune) {
        ...
        if (legend && !DeckManager.domainsMatch(legendDomains, rune.domains)) {
          errors.push(
            `Rune "${rune.name}" has domains [${rune.domains.join(', ')}] — does not share any domain with legend "${legend.name}" [${legendDomains.join(', ')}].`,
          );
        }
    ...
    const seenBattlefieldNames: Record<string, number> = {};
    for (const bfId of deck.battlefieldIds) {
      ...
      if (!bf) {
        ...
        if (seenBattlefieldNames[bf.name] > 1) {
          errors.push(
            `Battlefield "${bf.name}" appears ${seenBattlefieldNames[bf.name]} times (max 1 per name, 103.4.c).`,
          );
        }
        if (legend && !DeckManager.domainsMatch(legendDomains, bf.domains, true)) {
          errors.push(
            `Battlefield "${bf.name}" has domains [${bf.domains.join(', ')}] — does not share any domain with legend "${legend.name}" [${legendDomains.join(', ')}].`,
          );
        }
      }
    }
    ...
    const sbNameCount: Record<string, number> = {};
    for (const sbId of deck.sideboardIds) {
      const card = CARDS[sbId];
      if (!card) {
        ...
        if (legend && !DeckManager.domainsMatch(legendDomains, card.domains)) {
          errors.push(
            `Sideboard card "${card.name}" has domains [${card.domains.join(', ')}] — does not share any domain with legend "${legend.name}" [${legendDomains.join(', ')}].`,
          );
        }
        sbNameCount[card.name] = (sbNameCount[card.name] ?? 0) + 1;
        if (sbNameCount[card.name] > 3) {
          errors.push(
            `Sideboard has ${sbNameCount[card.name]} copies of "${card.name}" (max 3).`,
          );
        }
      }
    }
